#!/usr/bin/env tsx
/**
 * 9999-catalog cleanup, batch 8 — finish the 住 major (its 6 unranked subs).
 *
 * 正名 (assign a real sort_order — slotted into gaps, no existing rows renumbered):
 *   住:家電  → 60
 *   住:家具  → 65
 *   住:寢具  → 70
 *   住:其他  → 100   (NEW — 住 misc bucket, sorts last)
 *
 * Fold (sub retired, specific survives as a plain tag; existing notes preserved):
 *   住:仲介費    → 住:租金   + tag 仲介費   (1 row, rental agent fee)
 *   住:修繕費    → 住:其他   + tag 修繕費   (1 row)  ┐ one-time home services
 *   住:居家清潔  → 住:其他   + tag 居家清潔 (1 row)  ┘
 *
 * After this, 住 has zero sort_order=9999 subs.
 *
 * Idempotent. Read-only by default; set APPLY=1 to write.
 *   cd backend && npx tsx scripts/migrate-9999-batch8.ts          # dry run
 *            APPLY=1 npx tsx scripts/migrate-9999-batch8.ts       # execute
 */
...
const RANK: [string, number][] = [
  ['住:家電', 60], ['住:家具', 65], ['住:寢具', 70], ['住:其他', 100],
];
...
async function main() {
  console.log(`\n=== migrate-9999-batch8 (${APPLY ? 'APPLY' : 'DRY RUN'}) ===\n`);
  const all: Tx[] = [];
