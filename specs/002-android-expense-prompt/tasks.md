...
- [X] T001 Create `PendingManualInput` Room entity in `android/app/src/main/java/com/expenses/db/PendingManualInput.kt` — fields: id (autoincrement), text, parentTransactionId?, createdAt (epoch ms), retryCount
- [X] T002 [P] Create `PendingManualInputDao` in `android/app/src/main/java/com/expenses/db/PendingManualInputDao.kt` — methods: insert, getAll (ordered by createdAt ASC), delete(id), incrementRetry(id)
- [X] T003 Update `LocalDatabase` in `android/app/src/main/java/com/expenses/db/LocalDatabase.kt` — add `PendingManualInput::class` to @Database entities list, bump version 1→2, add Migration(1,2) that creates `pending_manual_inputs` table
- [X] T004 [P] Add `InputResponse`, `BudgetSummary`, and `CandidateTransaction` TypeScript types to `backend/src/types.ts` — see data-model.md for field definitions
