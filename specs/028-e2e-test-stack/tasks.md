...
- [ ] T007 [P] Add an `[env.e2e]` block to `backend/wrangler.toml` and create `backend/.dev.vars.e2e` with the local-only values from contracts/test-harness.md C2 (`SUPABASE_URL`, static local `SUPABASE_SERVICE_ROLE_KEY`, `ANDROID_API_KEY=e2e-test-key`, `PWA_ORIGIN=http://localhost:5300`).
- [ ] T008 Bring the stack up and load schema + catalog: from `backend/`, `supabase start` then `supabase db reset`; verify `categories` is populated (~133 rows) and the REST API answers at `http://127.0.0.1:54321`.
