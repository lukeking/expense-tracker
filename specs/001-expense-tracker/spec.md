...
- **FR-013**: The system MUST support a `fee [amount] [description]` command in both the chat interface and the Android prompt that creates a new linked fee transaction (國外交易服務費 or any deferred charge) referencing a prior transaction matched by description. If multiple candidates match, the user is presented with a selection list. The original transaction record is never modified.
- **FR-014**: The system MUST support a `refund [amount] [description]` command in both the chat interface and the Android prompt that creates a linked refund transaction (type: `refund`) referencing a prior transaction. Partial refunds are supported — the refund amount may be less than the original. Budget calculations for the period subtract refund amounts from gross spending: **net spend = Σ(expense) + Σ(fee) − Σ(refund)**. Business reimbursements (出公差墊付) and ticket cancellations use the same command.

### Key Entities

- **Transaction**: A single expense record. Attributes: unique ID, transaction type (`expense` / `refund` / `fee`), amount (NT$ positive integer), items (structured list of item name + sub-amount), payment method, wallet (optional), bank name (optional), tags (free-form list), matched-invoice flag, matched invoice reference, parent transaction reference (optional — set for `fee` and `refund` child records), chat message reference, timestamps.
...
- Q: How should refunds (退款/退票/請領) be modeled and affect budget? → A: `transaction_type` field added to all transactions (`expense` | `refund` | `fee`). Refunds are always positive-amount child transactions linked via `parent_transaction_id`; partial refunds supported. Budget = Σ(expense) + Σ(fee) − Σ(refund) for the period. Business reimbursements and ticket cancellations use the same `refund` command. Cancellation fees use the `fee` command.
