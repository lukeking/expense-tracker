...
- **FR-005**: Subcategory amounts (the header period total, each day's subtotal, and any per-transaction figure shown) MUST be computed from the **net per-item amount** (`effective_amount`) of the items matching the subcategory — not the whole-transaction amount — so amounts reflect actual subcategory spend (Goal 2) and reconcile with the day list. This is exact for item-tagged spend (the normal case); transactions tagged only at the transaction level are the known minor exception (see Edge Cases).
