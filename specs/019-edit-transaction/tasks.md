...
- [x] T001 Extend `replaceTransactionItems` item parameter type in `backend/src/db/queries.ts` — add `note?: string | null` to the items array element type (no body change needed; `insertTransactionItems` already handles it)
